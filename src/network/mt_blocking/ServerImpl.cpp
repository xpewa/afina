...
void ServerImpl::Stop() {
    ...
    for (auto client : _clients_socket) {
...
void ServerImpl::Join() {
...

    {
        ...
        while (running.load() && !_clients_socket.empty()) {
            _cv.wait(unique);
        }
    }
...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        {
            std::lock_guard<std::mutex> guard(_mutex);
            if (_clients_socket.size() >= _max_client) {
                _logger->error("Failed to connect. The maximum number of clients.");
                close(client_socket);
            }
            else {
                _clients_socket.insert(client_socket);
                std::thread new_worker = std::thread(&ServerImpl::ProcessConnection, this, client_socket);
                new_worker.detach();
            }
        }
    }
...
void ServerImpl::ProcessConnection(int client_socket) {
    ...
    std::lock_guard<std::mutex> guard(_mutex);
    _clients_socket.erase(client_socket);
    close(client_socket);
    if (!running.load() && _clients_socket.empty()) {
        _cv.notify_all();
    }
}
